...
async def get_documents(doc_id:int):
    doc = documentServices.find_docements_by_id(doc_id)
    if doc == None:
        raise HTTPException(status_code=404,detail='doc not found')
    return doc

@router.get('/doc/date/', status_code=status.HTTP_200_OK)
async def get_documents_date(start_date: Optional[str] = None , end_date: Optional[str] = None):
    try:
