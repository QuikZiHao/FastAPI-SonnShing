...
@router.post('/doc/',user_code=user.HTTP_202_CREATED)
async def create_user(user:UserBase):
    userServices.add_user(user=user)

@router.post('/docList/',user_code=user.HTTP_202_CREATED)
async def create_users_list(users:List[userBase]):
    userServices.add_userList(users=users)

@router.get('/doc/{doc_id}',user_code=user.HTTP_200_OK)
...
@router.get('/doc/date/', user_code=user.HTTP_200_OK)
async def get_users_date(start_date: Optional[str] = None , end_date: Optional[str] = None):
    try:
        ...
        if not users:
            raise HTTPException(user_code=404, detail="No users found")
        return {"users":users}
    except ValueError:
        raise HTTPException(user_code=400, detail="Invalid date format. Dates must be in YYYY-MM-DD format.")

# need to add check the branch valid or not
@router.get('/doc/branch/', user_code=user.HTTP_200_OK)
...
@router.delete('/doc/{doc_id}',user_code=user.HTTP_200_OK)
